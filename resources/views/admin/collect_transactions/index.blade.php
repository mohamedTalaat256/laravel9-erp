...
<div class="card">
    ...
    <form action="{{ route('admin.collect_transaction.store') }}" method="post" class="custom_form">
      <div class="row">
        ...
        <div class="col-md-4 > 
          ...
          <input type="date" name="move_date" id="move_date" class="form-control" value="{{ old('move_date',date("
            Y-m-d")) }}">
          @error('move_date')
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <select name="account_number" id="account_number" class="form-control select2 ">
              ...
              <option data-type="{{ $info->account_type }}" @if(old('account_number')==$info->account_number)
                selected="selected" @endif value="{{ $info->account_number }}"> {{ $info->name }} ({{
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <select name="mov_type" id="mov_type" class="form-control  ">
              ...
              <option @if(old('mov_type')==$info->id) selected="selected" @endif value="{{ $info->id }}"> {{ $info->name
                }} </option>
              @endforeach
              @endif
            </select>
        ...
        <div class="col-md-4">
          <div class="form-group">
            ...
            <select name="treasuries_id" id="treasuries_id" class="form-control ">
              <option value="{{ $checkExistsOpenShift['treasuries_id'] }}"> {{ $checkExistsOpenShift['treasuries_name']
                }} </option>
            </select>
            @error('treasuries_id')
            <span class="text-danger">{{ $message }}</span>
        ...
        <div class="col-md-4 > 
          ...
          <input oninput="this.value=this.value.replace(/[^0-9.]/g,'');" name="money" id="money" class="form-control"
            value="{{ old('money') }}">
          @error('money')
          <span class="text-danger">{{ $message }}</span>
          @enderror
        ...
        <div class="col-md-8">
          <div class="form-group">
            ...
            <textarea name="byan" id="byan" class="form-control" rows="4"
              cols="10"> {{ old("byan","تحصيل نظير ") }} </textarea>
            @error('byan')
            <span class="text-danger">{{ $message }}</span>
            @enderror
          </div>
  ...
  <div class="row" style="padding: 15px;">
    ...
    <div class="col-md-4">
      ...
      <input style="margin-top: 6px !important;" type="text" id="search_by_text" placeholder="" class="form-control">
      <br>
